...
controller_version="2.3.12"
promtail_image="docker.io/grafana/promtail:3.6.11"
loki_image="docker.io/grafana/loki:2.9.17"
prometheus_image="docker.io/prom/prometheus:v3.13.2"
grafana_image="docker.io/grafana/grafana:13.1.3"
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-nethsecurity-controller; then
    ...
    buildah from --name nodebuilder-nethsecurity-controller -v "${PWD}:/usr/src:Z" docker.io/library/node:24.18.1-slim
fi
