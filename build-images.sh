...
grafana_image="docker.io/grafana/grafana:12.4.5"
...
pushd build
webssh=$(buildah from docker.io/python:3.13.14-alpine)
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-nethsecurity-controller; then
    ...
    buildah from --name nodebuilder-nethsecurity-controller -v "${PWD}:/usr/src:Z" docker.io/library/node:24.16.0-slim
fi
