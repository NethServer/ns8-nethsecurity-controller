...
controller_version="2.3.3"
promtail_image="docker.io/grafana/promtail:3.6.10"
loki_image="docker.io/grafana/loki:2.9.17"
prometheus_image="docker.io/prom/prometheus:v3.10.0"
grafana_image="docker.io/grafana/grafana:12.4.2"
timescale_image="docker.io/timescale/timescaledb:2.23.1-pg16"
...
pushd build
webssh=$(buildah from docker.io/python:3.13.13-alpine)
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-nethsecurity-controller; then
    ...
    buildah from --name nodebuilder-nethsecurity-controller -v "${PWD}:/usr/src:Z" docker.io/library/node:24.14.1-slim
fi
