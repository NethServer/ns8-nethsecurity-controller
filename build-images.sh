...
controller_version="2.2.3"
promtail_image="docker.io/grafana/promtail:3.6.6"
loki_image="docker.io/grafana/loki:2.9.17"
prometheus_image="docker.io/prom/prometheus:v3.7.3"
grafana_image="docker.io/grafana/grafana:12.3.3"
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-nethsecurity-controller; then
    ...
    buildah from --name nodebuilder-nethsecurity-controller -v "${PWD}:/usr/src:Z" docker.io/library/node:24.14.0-slim
fi
